...
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="space-y-6 animate-in fade-in duration-700">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="cursor-pointer">
                        <input type="radio" name="status" value="active" class="peer hidden" {{ $category->status == 'active' ? 'checked' : '' }}>
                    ...
                    <label class="cursor-pointer">
                        <input type="radio" name="status" value="inactive" class="peer hidden" {{ $category->status == 'inactive' ? 'checked' : '' }}>
