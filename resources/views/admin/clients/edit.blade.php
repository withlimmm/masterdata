...
<div class="max-w-4xl mx-auto pb-20">
    ...
    <form action="{{ route('admin.clients.update', $client->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 animate-in fade-in duration-700">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white border border-slate-200 rounded-[2.5rem] p-8 md:p-10 shadow-sm space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="client_name" id="client_name" required value="{{ old('client_name', $client->client_name) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="testimonial" id="testimonial" rows="4"
                            class="w-full bg-slate-50 border border-slate-200 rounded-[1.5rem] px-6 py-5 text-sm leading-relaxed focus:bg-white focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all outline-none resize-none">{{ old('testimonial', $client->testimonial) }}</textarea>
                    </div>

                    <div class="space-y-2" x-data="{ rating: {{ old('rating', $client->rating ?? 5) }}, hover: 0 }">
            ...
            <div class="lg:col-span-4 space-y-8">
                ...
                <div class="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm space-y-6">
                    ...
                    <div class="relative group cursor-pointer" onclick="document.getElementById('company_logo').click()">
                        <div id="logoPreview" class="w-full aspect-square rounded-2xl bg-slate-50 border-2 border-slate-100 flex flex-col items-center justify-center text-slate-400 overflow-hidden transition-all hover:border-primary/50 group-hover:bg-slate-100/50">
                            @if($client->company_logo)
                                <img src="{{ asset('storage/' . $client->company_logo) }}" class="w-full h-full object-contain p-4">
                            @else
                                <span class="material-symbols-outlined text-4xl mb-2">add_photo_alternate</span>
                                <span class="text-[10px] font-black uppercase tracking-widest text-center px-4">Ganti Logo Perusahaan</span>
                            @endif
                        </div>
                ...
                <div class="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        <label class="cursor-pointer">
                            <input type="radio" name="status" value="active" class="peer hidden" {{ $client->status == 'active' ? 'checked' : '' }}>
                        ...
                        <label class="cursor-pointer">
                            <input type="radio" name="status" value="inactive" class="peer hidden" {{ $client->status == 'inactive' ? 'checked' : '' }}>
