...
@section('title', 'Tambah Banner - Rakira CMS')
@section('page_title', 'Tambah Banner')
@section('page_subtitle', 'Tambahkan banner promosi baru untuk halaman utama.')
...
<div class="max-w-4xl space-y-6">
    ...
    <form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="bg-white border border-slate-200 shadow-sm rounded-[2rem] p-8 md:p-10 space-y-8">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="file" name="image" required accept="image/*"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-4 focus:ring-slate-800/5 focus:border-slate-800 transition-all outline-none">
                    <p class="text-xs text-slate-400 mt-2">Disarankan ukuran lanskap (misal: 1200x400px), max 2MB.</p>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', 1) }}" required min="0"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-4 focus:ring-slate-800/5 focus:border-slate-800 transition-all outline-none">
